...
function getEddCustomerTotals($user_id = 0) {
	global $wpdb;
	...
	if ( $user_id ) {
        ...
        if (empty($orders)) { return $totals; }
// Инициализируйте переменные для расчетов
        $ltv = 0; // Сумма
        $total_order_value = 0; // Общая стоимость заказов
        $orders_count = count($orders); // Количество заказов

// Пройдитесь по каждому заказу
        foreach ($orders as $order) {
            $order_id = $order->ID;

            // Получите стоимость заказа
            $order_total = edd_get_payment_amount($order_id);
