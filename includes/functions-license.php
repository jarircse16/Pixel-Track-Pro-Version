		...
		<div class="row mt-3">
			<div class="col">
				<div class="alert alert-danger mb-0">
                    ...
                    <p>If you renewed your license but you still see this message, click on the "Reactivate License" button.</p>
					<p class="mb-0"><a href="https://www.webmxt.com/checkout/?edd_license_key=<?php esc_attr_e(
        ...
        if ( $license_data->success ) {
            ...
            switch ( $license_data->license ) {
                ...
                case 'expired':                 // license has expired
                    $admin_notice = array(
                        'class' => 'danger',
                        'msg'   => 'Your License has expired. <a href="http://www.webmxt.com/checkout/?edd_license_key=' . urlencode( $license_key ) . '&utm_campaign=admin&utm_source=licenses&utm_medium=renew" target="_blank">Renew it now.</a>'
                    );
                    break;
...
function singleCheckLicense( $license_key, $plugin)
{
    ...
    $response = wp_remote_post( 'https://www.webmxt.com', array(
        'timeout'   => 30,
        'sslverify' => false,
        'body'      => $api_params
    ) );
...
function updateLicense( $plugin ) {
	...
	$license_action = sanitize_text_field($_POST['pys'][ $slug ]['license_action']);
	$license_key    = isset( $_POST['pys'][ $slug ]['license_key'] ) ? $_POST['pys'][ $slug ]['license_key'] : $plugin->getOption( 'license_key' );
