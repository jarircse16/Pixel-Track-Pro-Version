...
class ServerEventHelper {
    ...
    private static function getUserData($wooOrder = null,$eddOrder = null) {
        ...
        if ( PixelYourSite\isWooCommerceActive() && isEventEnabled( 'woo_purchase_enabled' ) &&
            ($wooOrder || ( PixelYourSite\PYS()->woo_is_order_received_page() && wooIsRequestContainOrderId() ))
        ) {
        ...
        return apply_filters("ptp_fb_server_user_data",$userData);
    }
        ...
        if ( $user->ID ) {
            ...
            if(isset($_COOKIE['pys_advanced_form_data'])) {
                ...
                if(isset($advancedForm["email"]) && $advancedForm["email"] != "") {
                    $userData->setEmail($advancedForm["email"]);
                }
                if(isset($advancedForm["phone"]) && $advancedForm["phone"] != "") {
                    $userData->setPhone($advancedForm["phone"]);
                }
                if(isset($advancedForm["first_name"]) && $advancedForm["first_name"] != "") {
                    $userData->setFirstName($advancedForm["first_name"]);
                }
                if(isset($advancedForm["last_name"]) && $advancedForm["last_name"] != "") {
                    $userData->setLastName($advancedForm["last_name"]);
                }
            }
